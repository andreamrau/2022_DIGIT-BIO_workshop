...
Installation of all packages should take approximatey ~15 min.
...
[GitHub repo](https://github.com/andreamrau/2022_DIGIT-BIO_workshop) and can be 
...
`r Biocpkg("coseq")` clustering results. We start with the GMM results,
and examine boxplots of the co-expression clusters (with results 
aggegated by status group in the visualization) as well as the conditional 
probabilities of cluster membership for each cluster. Note that clusters that
